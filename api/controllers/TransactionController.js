...
const TransactionController = {
  ...
  payDebt: (req, res) => {
    ...
    TransactionService.payDebt(req.body, creditId)
    .then(result => {
      return ResponseService.json(200, res, `Fetched Transaction Successfully`, result);
    }) .catch(err => {
      sails.log.debug(`Transaction: Fetch Error -> ${JSON.stringify(err)}`);
  ...
  grantCreditRequest: (req, res) => {
    const creditRequestId = req.params.creditRequestId;
    const body = _.omit(req.body, ['creditorId']);
    const creditorId = req.body.creditorId;
    TransactionService.grantCreditRequest(body, {creditRequestId, creditorId})
    .then(result => {
      return ResponseService.json(200, res, `Fetched Transaction Successfully`, result);
    }) .catch(err => {
      sails.log.debug(`Transaction: Fetch Error -> ${JSON.stringify(err)}`);
